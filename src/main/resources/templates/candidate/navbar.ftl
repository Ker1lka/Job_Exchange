<nav class="navbar navbar-expand-lg bg-body-tertiary">
    <div class="container-fluid">
        <a class="navbar-brand" href="/">JobExchange</a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse"
                data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent"
                aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link active" href="/candidate/vacancies">Vacancies</a>
                </li>
                <li class="nav-item">
                    <a href="/about-us" class="nav-link">Про нас</a>
                </li>
            </ul>
            <form class="d-flex ms-auto" method="get" action="/candidate/vacancies">
                <input class="form-control me-2" type="search" name="search" placeholder="Пошук вакансій..." aria-label="Search" value="${RequestParameters.search!''}">
                <button class="btn btn-outline-success" type="submit">Знайти</button>
            </form>
        </div>
    </div>
</nav>